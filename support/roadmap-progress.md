...
Updated 2026-09-19 UTC. The A–F roadmap remains active; no full-release,
...
| K | K0–K3 done (2026-09-19): frozen gate, 6-program / 61-step official Cloud Tasks corpus recorded by the Phase H harness's `tasks` profile, `crates/tasks-front` (queue registry, the four routes, the dispatcher with the official cadence, `/queueStats`, discovery from the Functions inventory), replay 61/61 at 0 mismatches with no divergence, three runs identical, Functions corpus 271/271 | K5: exact-candidate CI, consumer gates, release `0.1.0-next.9` |
| L | L1–L4 done (2026-09-19): service subsets, any project id, any listen host, `ui.enabled`, `singleProjectMode`, `--debug-log`, multiple Firestore databases with per-database rules and export, storage object form and absence, missing `.firebaserc`, and the wrapper derivation of the official CLI | L5: same candidate as K |
| M | M1–M4 done (2026-09-19): `emulators:export`, `init` / `init --adopt`, `use`, `target:apply` / `target:clear`, `firestore:delete`, `--debug`, compatibility flags, exec shell-string form, `functions:invoke --event-data`, `mcp`; 92 packaging tests | M5: same candidate as K |
