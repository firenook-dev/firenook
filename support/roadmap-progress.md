...
| I | I0–I5 done and published in `0.1.0-next.8` (2026-09-19): frozen gate, 61-program / 1068-step official Auth corpus, `auth-front` rebuilt on the bundled official OpenAPI document with every one of the 61 operations, replay 17,303 values at 0 mismatches with 4 named divergences, real-SDK popup/redirect browser gate, Emulator UI Auth-tab check, exact-candidate CI 35364256374, consumer gates and nine browser journeys on the candidate; paired soak waived (per-release-line gate) | Nothing for this release line; the 42 routes the official emulator answers with 501 stay 501 by design |
| J | J0–J5 done and published in `0.1.0-next.8` (2026-09-19): frozen gate, 42-program / 916-step official Pub/Sub corpus over gRPC, HTTP/JSON, streaming pull and push, `pubsub-front` rewritten as a broker behind tonic and an HTTP/JSON transcoder, replay 3,755 values at 0 mismatches with no divergence, `@google-cloud/pubsub` 5.3.1 client check 7/7, Functions runtime corpus 271/271, same qualification as I | Nothing for this release line; Protocol Buffer schemas, IAM, detach and updateSnapshot answer 501 as the official emulator does |
| K | K0–K3 done (2026-09-19): frozen gate, 6-program / 61-step official Cloud Tasks corpus recorded by the Phase H harness's `tasks` profile, `crates/tasks-front` (queue registry, the four routes, the dispatcher with the official cadence, `/queueStats`, discovery from the Functions inventory), replay 61/61 at 0 mismatches with no divergence, three runs identical, Functions corpus 271/271 | Nothing for this release line; published in `0.1.0-next.9` (2026-09-19) |
| L | L1–L4 done (2026-09-19): service subsets, any project id, any listen host, `ui.enabled`, `singleProjectMode`, `--debug-log`, multiple Firestore databases with per-database rules and export, storage object form and absence, missing `.firebaserc`, and the wrapper derivation of the official CLI | Nothing for this release line; published in `0.1.0-next.9` (2026-09-19) |
| M | M1–M4 done (2026-09-19): `emulators:export`, `init` / `init --adopt`, `use`, `target:apply` / `target:clear`, `firestore:delete`, `--debug`, compatibility flags, exec shell-string form, `functions:invoke --event-data`, `mcp`; 92 packaging tests | Nothing for this release line; published in `0.1.0-next.9` (2026-09-19) |
