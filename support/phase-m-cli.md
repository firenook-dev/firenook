...
Firenook's model: no login, no cloud project lookup, no second Functions
runtime.

## Decisions per command (from the official source, firebase-tools 15.22.0)

| Command | Official behaviour | Firenook |
| --- | --- | --- |
...
| `functions:shell` | Starts a second private Functions emulator and opens a REPL | Not provided: Firenook never runs a second Functions runtime; `functions:invoke` gains `--event-data` to deliver a background event to the running suite |
...
| `--debug` | `firebase-debug.log` in the cwd, deleted on a successful exit; per-emulator `<name>-debug.log` files | One file per run in `.firenook/runs/session-*/firenook-debug.log`, never deleted, every record labelled with its emulator |
