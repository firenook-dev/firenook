...
multi-factor and passkey flows, out-of-band (email) and phone verification
codes, session cookies, the emulator-only inspection routes and the two browser
helper pages. Blocking functions run on every sign-in path, not only the
password one. Export/import stays byte-compatible with the official
...
Center, `verifyIosClient`, IAM on tenants, reCAPTCHA enforcement, real email or
SMS delivery). Those keep answering as the official emulator answers them
(recorded in I1), and the Emulator UI is unchanged.
...
| Email OOB | none | `sendOobCode` (VERIFY_EMAIL, PASSWORD_RESET, EMAIL_SIGNIN, VERIFY_AND_CHANGE_EMAIL), `resetPassword`, `signInWithEmailLink`, `update` with `oobCode`, admin `accounts:sendOobCode`, `GET /emulator/v1/projects/{p}/oobCodes`, `/emulator/action` landing page, link in the log | whole feature |
...
| MFA / passkeys | none | `mfaEnrollment:start/finalize/withdraw`, `mfaSignIn:start/finalize` (SMS, TOTP), `passkeyEnrollment:start/finalize`, `passkeySignIn:start/finalize`, `mfaInfo`/`mfaPendingCredential` | whole feature |
| Blocking functions | `beforeCreate`/`beforeSignIn` for password `signUp`/`signInWithPassword` (`lib.rs:579,602,726`) | every sign-in method incl. IdP, custom token, phone, email link, anonymous; `beforeSendEmail`/`beforeSendSms` | coverage |
| Config | emulator config GET/PATCH stored as JSON; `allowDuplicateEmails` and `emailPrivacyConfig` not enforced | `signIn.allowDuplicateEmails`, `emailPrivacyConfig.enableImprovedEmailPrivacy` (`INVALID_LOGIN_CREDENTIALS` masking), `usageMode: PASSTHROUGH`, `blockingFunctions`, `mfa`, `passwordPolicy` via v2 config | enforcement |
| Misc | readiness advertises `/emulator/openapi.json` (route absent) | serves the spec; widget page with account picker, "add new account", tenant awareness | small |
...
- MFA: SMS enrolment/sign-in, TOTP enrolment/sign-in, withdraw, `mfaInfo` on
  records, `mfaPendingCredential`, sign-in with MFA required; passkeys start/
  finalize both flows (the emulator accepts any well-formed attestation).
- Blocking functions: `beforeCreate`/`beforeSignIn` on each sign-in method,
  response updates (`displayName`, `customClaims`, `sessionClaims`,
  `disabled`), errors, `beforeSendEmail`/`beforeSendSms`, the JWT sent to the
  function, `usageMode: PASSTHROUGH`.
...
- Errors: the 41 unimplemented operations (`501` shape), unknown routes, bad
  JSON, missing `Authorization` on admin routes, wrong project id.
...
- `model.rs`: `Account` (every field the official `UserInfo` carries:
  ...
  `disabled`, `tenantId`, `mfaInfo` (SMS + TOTP enrolments),
  passkeys), `ProjectState` with the official indexes (email, initial email,
  phone, provider raw id, passkey credential id), OOB codes, verification
  codes, temporary proofs, refresh tokens, session cookies, tenants (each a
  `ProjectState` with tenant config), project config (v2 shape).
- `handlers/*.rs` per operation group (password, idp, custom, oob, phone,
  update, admin, tenants, mfa, passkeys, config, emulator), each mapping the
  recorded validation order and error strings.
...
- One `run_blocking(event, method, account)` used by every sign-in path;
  `beforeSendEmail`/`beforeSendSms` before OOB and phone codes; the JWT
  payload as recorded; `PASSTHROUGH` skips the store.
- `trigger_multicast` for every create/delete path, silent for imports.
