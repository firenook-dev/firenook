...
// The owned admission adapter that this script once verified in-process lives
// in the Rust runtime now (crates/functions-runtime/src/registry.rs); the
// Functions runtime replay (`replay:functions:runtime`) is its verification.
assert(process.argv.length===6,'binary toolsRoot sdkRoot fresh-output');
...
try{
  ...
  for(const [mode,backends] of [['healthy',[http,auxiliary]],['failed-codebase',[http,broken]],['missing-auxiliary',[http,auxiliary]],['predefined-backend',[predefined]],['colliding-backends',[http,{...http,codebase:'collision'}]],['unsupported-predefined',[unsupported]]]){
    ...
    try{
      let connectError;
      try{await EmulatorRegistry.start(emulator);await emulator.connect();}
