...
export function syntheticRecoveryFixture() {
  ...
  const packages=[...platforms.map(name=>`@firenook/cli-${name}`),'firenook'].map(name=>({name,sha256:'a'.repeat(64),integrity:'sha512-synthetic'}));
  ...
  const fixture={run:{id:sourceRun,run_attempt:1,head_sha:sourceCommit,head_branch:'main',repository:{full_name:'firenook-dev/firenook'},path:'.github/workflows/release-npm.yml',event:'workflow_dispatch',status:'completed',conclusion:'failure'},jobs:[...names.map(name=>({name,conclusion:'success'})),{name:'publish',conclusion:'failure'}],artifacts:artifacts.map(artifact=>({...artifact,expired:false,workflow_run:{id:sourceRun,head_sha:sourceCommit,repository_id:1,head_repository_id:1}}))};
