...
const consumer = mkdtempSync(join(tmpdir(),`firenook ${manager} 中文 consumer-`));
writeFileSync(join(consumer,'package.json'),JSON.stringify({name:'firenook-install-smoke',private:true,version:'1.0.0'}));
...
const installedVersion = packageManager(manager, manager === 'npm' ? ['exec','--offline','--','firenook','--version'] : ['run','firenook','--version'], {cwd:consumer,encoding:'utf8'});
